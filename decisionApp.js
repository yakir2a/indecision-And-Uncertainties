...
*/
    ...
    switch (submitStage) {
      ...
      case "table_build":
        if (
          ...
          if (state.stage == "filling_indecision") {
            ...
            $(`#input_table`).attr(
              "onsubmit",
              'decision.stage("table_indecision_input",event);decision.removeBlur();'
            );
            state.fill.fillObjs();
            state.counter.increaseCount();
          } else if (state.stage == "filling_uncertainty") {
            $("form#input").append(
              `<h3>הכנס את מצבי אי הוודאות (בשורה העליונה) ואת הסתברותם (בשורה התחתונה)</h3>`
            );
            $("form#input").removeAttr("onsubmit");
            $(`#input_table`).attr(
              "onsubmit",
              'decision.stage("table_uncertainty_input",event);decision.removeBlur();'
            );
            state.fill.fillObjs();
            state.fill.fillObjs();
            state.counter.increaseCount();
            ...
            $(`#input_table`).attr(
              "onsubmit",
              'decision.stage("table_benefit_input",event);decision.removeBlur();'
            );
          }
      ...
      case "table_indecision_input":
        if (["initialize", "table_build"].includes(state.stage)) {
          console.log(
            `table_indecision_input stage, from ${state.stage} stage`
          );
      ...
      case "table_uncertainty_input":
        if (["table_build"].includes(state.stage)) {
          ...
          if (checkProbabilty()) {
            ...
            alert(
              "נא למלא את הסתברויות של כל אי ודאות בצורה נכונה(חיבור שלהם צריך להיות שווה 1)"
            );
          }
        }
        break;
  ...
  */
  ...
  var table_build = function (table_nodes, stage, index) {
    ...
    switch (stage) {
      case "filling_indecision":
        ...
        table_nodes.forEach(function (node) {
          ...
          for (i = 0; i < indecision; i++) {
            ...
            let addEvents = $(child.block).find("#input");
            addEvents.on("focus", () => {
              addBlur();
              mark_path(child);
            });
            addEvents.on("focusout", () => {
              removeBlur();
            });
      ...
      case "filling_uncertainty":
        ...
        table_nodes.forEach(function (node) {
          ...
          for (i = 0; i < uncertainty; i++) {
            ...
            let addEvents = $(child.block).find(
              "#input_uncertainty, #input_probabilty"
            );
            addEvents.on("focus", () => {
              addBlur();
              mark_path(child);
      ...
      case "table_benefit":
        ...
        table_nodes.forEach(function (node) {
          node.benefit = $(
            `<tr><td><input id="input_probabilty" type="number" ${mode.limit} required></td></tr>`
          );
  ...
  //marks the path of the focuse element
  function mark_path(node) {
    for (; node.parent != undefined; node = node.parent) {
      $(node.block).find("tbody:first>tr:not(:last-child)").removeClass("blur");
    }
    $(node.block).find("tbody:first>tr:not(:last-child)").removeClass("blur");
  }
  ...
  function probability(input) {
    let constant = 0.61;
    let v = Math.pow(input, constant);
    let p = Math.pow(1 - input, constant);
    return v / Math.pow(v + p, 1 / constant);
  }

  var removeBlur = function () {
    $("#decision tbody>tr:not(:last-child)").removeClass("blur");
  };

  var addBlur = function () {
    console.log($("#decision tbody>tr:not(:last-child)"))
    $("#decision tbody>tr:not(:last-child)").addClass("blur");
  };
    ...
    var fold = setInterval(
      function () {
        ...
        current.forEach(function (parent) {
          if (parent.parent != undefined) next.push(parent.parent);

          switch (parent.childType) {
            case "decision":
              //----fold decision
              const pick = parent.childs.reduce(function (prev, current) {
                return parseFloat($(prev.benefit).text()) >
                  parseFloat($(current.benefit).text())
                  ? prev
                  : current;
              });
              parent.childs = [pick];
            ...
            case "uncertainty":
              //----fold uncertainty
              const benefit = parent.childs
                .map(function (child) {
                  return (
                    probability(
                      parseFloat(child.block.find(`#input_probabilty`).text())
                    ) * parseFloat(child.benefit.text())
                  );
                })
              ...
              $(parent.block)
                .find("#childs")
                .replaceWith('<table style="width:100%" id="fill"></table>');
              var tmp = "";
  ...
  var showResult = function () {
    const picked = state.indecision.reduce(function (prev, current) {
      a = parseFloat(parseFloat($(prev.benefit).text()).toFixed(3));
      b = parseFloat(parseFloat($(current.benefit).text()).toFixed(3));
      return a > b ? prev : current;
    });
    var result = $(`<br><table class="result"><tr></tr></table>`);
    var tmp = result.find("tr");
    create_result_table(picked, tmp, 0);
    $("#tables").append("<h3>הראה את הפתרון:<h3><hr>");
    $("#tables").append(result);
    $("#tables").append(
      `<br><h3 style="{'color':'black'}">תוחלת ה${mode.result}: ${picked.benefit.text()}<h3>`
    );
  };
  ...
  var create_result_table = function (node, element, index) {
    ...
    if (node.type == "decision") {
      if (!node.childs.length || node.childType == "fill") {
        element.append(
          `<td><b style="color:blue">${$(node.block)
            .find("#input_decision")
            .text()}</b></td>`
        );
        return;
        ...
        element.append(
          `<td><b style="color:black">${$(node.block)
            .find("#input_decision")
            .text()} ---></b></td>`
        );
      ...
      if (!node.childs.length || node.childType == "fill") {
        element
          ...
          .html(element.find("td:last-child b").text().replace(" --->", ""));
        return;
      }
      element.append(
        `<td><b style="color:red">אם ${$(node.block)
          .find("#input_uncertainty")
          .text()} אז ---></b></td>`
      );
      create_result_table(node.childs[0], element, index);
    }
    for (i = 1; i < node.childs.length; i++) {
      tmp = $("<tr></tr>");
      for (j = 0; j < index; j++) tmp.append("<td> </td>");
      element.after(tmp);
      create_result_table(node.childs[i], tmp, index);
    }
  };

  return { stage, removeBlur, addBlur, pickMode};
})();
